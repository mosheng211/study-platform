...
<body>
    <div class="container">
        <h1>🎓 学习资源批量导入工具</h1>
        <div class="info">
            <strong>54天编程学习计划 - 资源导入系统</strong><br>
            本工具将导入精选的中文学习资源，涵盖Web技术、C#、Vue.js和浏览器自动化等技术栈
        </div>

<?php
if (isset($_POST['import'])) {
    echo '<div class="status info">开始导入学习资源...</div>';
    echo '<pre>';
    ...
    try {
        ...
        echo '</pre>';
        echo '<div class="status success"><strong>导入成功！</strong> 已成功导入 ' . $success_count . ' 个学习资源</div>';
        ...
        echo '<div class="stat-card"><div class="stat-number">' . $totalResources . '</div><div class="stat-label">总资源数</div></div>';
        echo '<div class="stat-card"><div class="stat-number">' . $totalCategories . '</div><div class="stat-label">分类数</div></div>';
        echo '<div class="stat-card"><div class="stat-number">' . $success_count . '</div><div class="stat-label">本次导入</div></div>';
        echo '</div>';
        
    } catch (Exception $e) {
        echo '</pre>';
        echo '<div class="status error">❌ 错误: ' . $e->getMessage() . '</div>';
        echo '<div class="status info">请检查数据库连接和表结构。</div>';
    }
} else {
    // 显示导入前的统计信息（如果可以连接数据库）
    try {
        $pdo = new PDO('mysql:host=localhost;dbname=study_platform;charset=utf8mb4', 'root', '61263269');
        $stmt = $pdo->query("SELECT COUNT(*) FROM resources");
        ...
        echo '<div class="stats">';
        echo '<div class="stat-card"><div class="stat-number">' . $currentCount . '</div><div class="stat-label">当前资源数</div></div>';
        echo '<div class="stat-card"><div class="stat-number">21</div><div class="stat-label">待导入资源</div></div>';
        echo '<div class="stat-card"><div class="stat-number">7</div><div class="stat-label">学习分类</div></div>';
        echo '</div>';
        
    } catch (Exception $e) {
        echo '<div class="status error">无法连接到数据库: ' . $e->getMessage() . '</div>';
    }
    
    echo '
    <form method="post" style="text-align: center; margin: 30px 0;">
        <button type="submit" name="import" class="btn">🚀 开始导入学习资源</button>
    </form>
    
    <div class="info">
        <h3>📋 将导入的资源包括：</h3>
        <ul style="text-align: left;">
            <li><strong>Web基础 (4个)</strong>: HTML/CSS教程、前端基础视频课程</li>
            <li><strong>JavaScript (3个)</strong>: 廖雪峰教程、ES6教程、高级程序设计</li>
            <li><strong>C# (4个)</strong>: 微软官方文档、菜鸟教程、视频课程</li>
            <li><strong>Vue.js (3个)</strong>: 官方文档、实战教程、全家桶课程</li>
            <li><strong>自动化测试 (2个)</strong>: Selenium教程和实战课程</li>
            <li><strong>CEF3 (2个)</strong>: CefSharp文档、C#浏览器自动化实战</li>
            <li><strong>在线工具 (3个)</strong>: CodePen、CSDN、GitHub项目</li>
        </ul>
    </div>';
}
?>
        ...
        <div style="text-align: center; margin-top: 30px;">
            ...
            <p>
                <a href="http://localhost:5173/resources" target="_blank">📚 前端资源页面</a> | 
                <a href="http://localhost:5173/admin" target="_blank">⚙️ 后台管理</a> | 
                <a href="http://localhost:8000/api/resources" target="_blank">🔗 资源API</a>
            </p>
        </div>
    </div>
</body>
